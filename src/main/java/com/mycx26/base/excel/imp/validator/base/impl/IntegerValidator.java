...
public class IntegerValidator extends ValueValidator {
    ...
    public void validate(List<String> values, Template template, StringBuilder error) {
        List<Integer> orders = template.getColTypeOrdersMap().get(ColTypeEnum.INTEGER.getCode());
        String value;
        ...
        for (Integer order : orders) {
            ...
            if (!pattern.matcher(value).matches()) {
                error.append(ExcelMsgUtil.appendColMsg(order - 1, value)).append("不是整数").append(StringUtil.LF);
            }
        }
    }
}
